...
const Introduction = () => {
  ...
  return (
    <div className="safe-screen bg-[url(evening.jpg)] bg-cover bg-center text-white overflow-x-hidden">
      <div className="align-center flex flex-col md:flex-row min-h-screen justify-between mx-4 md:mx-8 relative">
        <div className=" p-6 md:p-8 md:w-1/2 w-full">
          ...
          <p className="leading-relaxed px-4 md:px-8 pt-3 text-base sm:text-sm md:text-lg lg:text-xl">
            hi, nice to meet you! I am a brazilian girl living in japan looking for new opportunities. I am a little bit shy and an introvert.
            I am a future web developer, and I hope to learn much more as time passes by.
          </p>
            ...
            {contents.map((c, index )=> {
              return (
                <li className="list-none py-2 md:py-4 text-2xl sm:text-2xl md:text-3xl hover:text-[#FBAF00] transition">
                  <Link to={`/${c.path}`} key={index}>{c.label}</Link>
                </li>
              )
            })}
          </ul>
